    ...
    <section class="container_box">
      ...
      $rowa =  $resulta->fetch_assoc();
       ...
       /*if($row_absen['status']=='Telat'){
        ...
        $durasi_kerja_start = strtotime(''.$date_month_year.' '.$row_absen['time_in'].'');
		$durasi_kerja_end   = strtotime(''.$date_month_year.' '.$row_absen['time_out'].'');
		$diff  				= $durasi_kerja_end - $durasi_kerja_start - 3600;
		$durasi_jam   		= floor($diff / (60 * 60));
		$durasi_menit 		= $diff - ($durasi_jam * (60 * 60) );
		$durasi_detik 		= $diff % 60;
		$durasi_kerja 		= ''.$durasi_jam.' jam, '.floor($durasi_menit/60 ).' menit';
    ...
    <section class="container_box">
      ...
      $rowa =  $resulta->fetch_assoc();
        ...
        if($row_absen['status']=='Telat'){
          ...
          // DURASI KERJA  =========================================
        $time_in 			= $row_absen['time_in'];
        $time_out 			= $row_absen['time_out']; 
        $durasi_kerja_start = strtotime(''.$date_month_year.' '.$time_in.'');
        $durasi_kerja_end   = strtotime(''.$date_month_year.' '.$time_out.'');
        $diff  				= $durasi_kerja_end - 3600 - $durasi_kerja_start;
        $durasi_jam   		= floor($diff / (60 * 60));
         if ($durasi_jam  < 0) {
        	$durasi_jam  = 0;
        }
        ...
        $durasi_kerja 		= ''.$durasi_jam.' jam, '.floor($durasi_menit/60 ).' menit';
          ...
          // DURASI KERJA keseluruhan =========================================
        $time_in 				= $row_absen['time_in'];
        $time_out 				= $row_absen['time_out']; 
        $durasi_kerja_start_all = strtotime(''.$date_month_year.' '.$time_in.'');
        $durasi_kerja_end_all   = strtotime(''.$date_month_year.' '.$time_out.'');
        $diff_all  				= array_sum($durasi_kerja_end_all) - 3600 - array_sum($durasi_kerja_start_all);
        $durasi_jam_all   		= floor($diff_all / (60 * 60));
         if ($durasi_jam_all  < 0) {
        	$durasi_jam_all  = 0;
        }
        $durasi_menit_all 		= $diff_all - ($durasi_jam_all * (60 * 60) );
         if ($durasi_menit_all  < 0) {
        	$durasi_menit_all  = 0;
        }
        $durasi_detik_all 		= $diff_all % 60;
        $durasi_kerja_all 		= ''.$durasi_jam_all.' jam, '.floor($durasi_menit_all/60 ).' menit';        
         
         
        // JAM LEMBUR =========================================
        if($row_absen['time_out'] > $shift_time_out ){
          $lembur_kerja_start_time = strtotime(''.$date_month_year.' '.$shift_time_out.'');
          $lembur_kerja_start = $lembur_kerja_start_time + 4500;
          $lembur_kerja_end   = strtotime(''.$date_month_year.' '.$row_absen['time_out'].'');
          $diff  				= $lembur_kerja_end - $lembur_kerja_start;
          $lembur_jam   		= floor($diff / (60 * 60));
        	if ($lembur_jam  < 0) {
          ...
          $lembur 				= ''.abs($lembur_jam).' jam, '.floor($lembur_menit/60 ).' menit';
        ...
        echo'
         <tr style="background:'.$background.';color:'.$warna.'">
            ...
            echo'
              <td class="text-center">'.$shift_time_out.'</td>
              <td class="text-center">'.$row_absen['time_out'].'</td>
              <td class="text-center">'.$row_absen['selisih_out'].'</td>';
            }
            echo'
    		...
    		echo'
    	  		<tr>
      				<th colspan="8">Total Jam Kerja</th>
