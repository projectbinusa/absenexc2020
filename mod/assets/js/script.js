...
$('#update-password').submit(function (e) {
    ...
    if($('#employees_password').val()==''){    
        ...
        loading();
        $.ajax({
            url:"./proses?action=update-password",
            type: "POST",
            data: new FormData(this),
            processData: false,
            contentType: false,
...
/* --------- UPDATE PHOTO PROFILE ---------------*/
 $(document).on('change','#avatar',function(){
        ...
        $.ajax({
          url:'./proses?action=update-photo',
          method:'POST',
          data:form_data,
          contentType:false,
          cache:false,
...
/* ------------------- UPDATE DATA HISTORY ------------------------- */
    ...
    /* ---------- UPDATE HISTORY-----------------*/
        $('#update-history').submit(function (e) {
            ...
            if($('#timein').val()=='' && $('#timeout').val()==''){    
                ...
                loading();
                $.ajax({
                    url:"./proses?action=update-history",
                    type: "POST",
                    data: new FormData(this),
                    processData: false,
                    contentType: false,
   ...
   $(document).on('click', '.btn-update-cuty', function(){
    ...
    function loadDataCounter() {
        $.ajax({
            url: './proses?action=load-home-counter',
            type: 'POST',
            success: function(data) {
              $('.load-home').html(data);
            }
...
function nWin(context,title) {
    ...
    doc.write("<link href='mod/assets/css/print.css' rel='stylesheet' type='text/css' media='print'>");
    doc.write("</head><body>");
    doc.write(context);
    doc.write("</body></html>");
    doc.close();
